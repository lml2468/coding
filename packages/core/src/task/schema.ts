/**
 * Canonical task.json shape — single source of truth for Coding tasks.
 *
 * The runtime Python writer is `.coding/scripts/common/task_store.py`
...
export const TASK_RECORD_FIELD_ORDER = [
  ...
  "notes",
  "meta",
] as const satisfies readonly (keyof CodingTaskRecord)[];
...
/**
 * Lightweight runtime schema for {@link CodingTaskRecord}. Zero-dep on
...
export const taskRecordSchema = {
  parse(input: unknown): CodingTaskRecord {
    return parseTaskRecord(input);
  },
  safeParse(
    input: unknown,
  ):
    | { success: true; data: CodingTaskRecord }
...
function parseTaskRecord(input: unknown): CodingTaskRecord {
...
/**
 * Produce a fully-populated canonical-shape {@link CodingTaskRecord}.
 *
...
export function emptyTaskRecord(
  overrides: Partial<CodingTaskRecord> = {},
): CodingTaskRecord {
  const today = new Date().toISOString().split("T")[0] ?? "";
  const base: CodingTaskRecord = {
    id: "",
    name: "",
    title: "",
    description: "",
