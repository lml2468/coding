import type { CodingTaskRecord } from "./schema.js";

/**
 * Coarse-grained Coding task phase derived from task status.
 *
 * Phase is a projection of {@link CodingTaskRecord.status} only. There is
...
export function inferTaskPhase(
  recordOrStatus: CodingTaskRecord | string | null | undefined,
): CodingTaskPhase {
  const status =
