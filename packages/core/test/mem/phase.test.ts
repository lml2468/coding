...
const { fakeHome } = vi.hoisted(() => {
  ...
  const fakeHome = f.mkdtempSync(p.join(o.tmpdir(), "coding-mem-phase-"));
  return { fakeHome };
});
...
describe("parseTaskPyCommand", () => {
  ...
  it('matches `python ./.coding/scripts/task.py create "foo"`', () => {
    const r = parseTaskPyCommand(
      'python ./.coding/scripts/task.py create "fix bug"',
    );
    expect(r).toEqual({
      action: "create",
      slug: undefined,
  ...
  it("matches `python3 ./.coding/scripts/task.py create ...`", () => {
    const r = parseTaskPyCommand(
      "python3 ./.coding/scripts/task.py create my-task",
    );
    expect(r?.action).toBe("create");
  });

  it("matches `py -3 .coding/scripts/task.py create ...` (Windows launcher)", () => {
    const r = parseTaskPyCommand("py -3 .coding/scripts/task.py create foo");
    expect(r?.action).toBe("create");
  });

  it("matches Windows backslash path (single)", () => {
    const r = parseTaskPyCommand(
      "python3 .coding\\scripts\\task.py start .coding\\tasks\\05-08-foo",
    );
    expect(r).toEqual({
      action: "start",
      taskDir: ".coding\\tasks\\05-08-foo",
    });
  });

  it("matches Windows backslash path (double — JSONL re-escape)", () => {
    const r = parseTaskPyCommand(
      "python3 .coding\\\\scripts\\\\task.py create my-task",
    );
    expect(r?.action).toBe("create");
  });

  it("matches `task.py start` with no invoker prefix", () => {
    const r = parseTaskPyCommand("task.py start .coding/tasks/05-08-foo/");
    expect(r).toEqual({
      action: "start",
      taskDir: ".coding/tasks/05-08-foo/",
    });
  });

  it("matches absolute path", () => {
    const r = parseTaskPyCommand(
      "python3 /Users/me/proj/.coding/scripts/task.py create new-thing",
    );
    expect(r?.action).toBe("create");
  });
  ...
  it("captures --slug FOO flag value", () => {
    const r = parseTaskPyCommand(
      'python3 .coding/scripts/task.py create "Title" --slug my-slug',
    );
  ...
  it("captures --slug=FOO equals form", () => {
    const r = parseTaskPyCommand(
      "python3 .coding/scripts/task.py create --slug=my-slug",
    );
    expect(r).toMatchObject({ action: "create", slug: "my-slug" });
  });
  ...
  it("does NOT match `task.py update` (only create/start are signals)", () => {
    expect(
      parseTaskPyCommand("python3 .coding/scripts/task.py update foo"),
    ).toBeNull();
  });
...
describe("parseTaskPyCommandsAll (dogfood-driven edge cases)", () => {
  it("strips $(...) closing paren from --slug value", () => {
    const all = parseTaskPyCommandsAll(
      'TASK_DIR=$(python3 ./.coding/scripts/task.py create "fix: tl mem --since drops cross-day sessions" --slug mem-since-cross-day-filter)',
    );
  ...
  it("captures BOTH task.py invocations in one Bash command", () => {
    const cmd =
      'SMOKE_TASK=$(python3 ./.coding/scripts/task.py create "smoke" 2>&1); python3 ./.coding/scripts/task.py start ".coding/tasks/$SMOKE_TASK" 2>&1 | tail -3';
  ...
  it("rejects prose-embedded matches (heredoc / commit-message text)", () => {
    const cmd =
      'git commit -m "Previous text said `.current-task` is a CLI fallback. Current code never writes that file — task.py start exits with hint to set CODING_CONTEXT_ID."';
...
describe("slugFromTaskDir (via buildBrainstormWindows pairing)", () => {
  it("pairs --slug FOO with start .coding/tasks/MM-DD-FOO via prefix strip", () => {
    const events: TaskPyEvent[] = [
      ...
      },
      {
        ...
        turnIndex: 10,
        taskDir: ".coding/tasks/05-08-mem-fix",
      },
    ];
...
describe("buildBrainstormWindows", () => {
  ...
  it("pairs multi-task FIFO when slugs are missing", () => {
    const events = [
      ev("create", 1),
      ev("start", 3, { taskDir: ".coding/tasks/aaa" }),
      ev("create", 5),
      ev("start", 9, { taskDir: ".coding/tasks/bbb" }),
    ];
    expect(buildBrainstormWindows(events, 12)).toEqual([
  ...
  it("prefers slug match over FIFO order", () => {
    const events = [
      ...
      ev("start", 5, { taskDir: ".coding/tasks/bbb" }),
      ev("start", 6, { taskDir: ".coding/tasks/aaa" }),
    ];
  ...
  it("fallback B: start with no preceding create → [0, start)", () => {
    const events = [ev("start", 7, { taskDir: ".coding/tasks/earlier" })];
    expect(buildBrainstormWindows(events, 12)).toEqual([
  ...
  it("skips malformed window where start.turnIndex < create.turnIndex (event order quirk)", () => {
    const events = [
      ...
      ev("start", 3, { taskDir: ".coding/tasks/weird" }),
    ];
    expect(buildBrainstormWindows(events, 10)).toEqual([]);
  });
        ...
        message: {
          ...
          content: [
            { type: "text", text: "creating the task now" },
            {
              ...
              input: {
                command:
                  'python3 ./.coding/scripts/task.py create "task X" --slug task-x',
              },
            },
          ],
        },
        ...
        message: {
          ...
          content: [
            { type: "text", text: "starting the task" },
            {
              ...
              input: {
                command:
                  "python3 ./.coding/scripts/task.py start .coding/tasks/task-x",
              },
            },
          ],
        },
        ...
        message: {
          ...
          content: [
            { type: "text", text: "creating" },
            {
              ...
              input: {
                command:
                  "python3 ./.coding/scripts/task.py create --slug post-compact",
              },
            },
          ],
        },
        ...
        message: {
          ...
          content: [
            { type: "text", text: "creating ahead of compact" },
            {
              ...
              input: {
                command:
                  "python3 ./.coding/scripts/task.py create --slug stale",
              },
            },
          ],
        },
