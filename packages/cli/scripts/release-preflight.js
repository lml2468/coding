 ...
 *   1. Version match between `@limenglin/coding` and
 *      `@limenglin/coding-core` (and the current git tag when checked from
 *      a tag context).
 ...
 *                                    on @limenglin/coding-core resolves
...
function verifyPackedCli() {
  ...
  try {
    ...
    const dep = packedPkg.dependencies?.["@limenglin/coding-core"];
    if (!dep) {
      fail(`packed CLI is missing dependency on @limenglin/coding-core.`);
    }
    if (dep !== v.cliVersion) {
      fail(
        `packed CLI depends on @limenglin/coding-core@"${dep}" but expected exact "${v.cliVersion}".\n` +
          `pnpm should rewrite workspace:* to the exact published version; got "${dep}" instead.`,
      );
    }
    console.log(
      `${GREEN}ok${RESET} packed CLI pins @limenglin/coding-core to exact ${v.cliVersion}.`,
    );
  } finally {
