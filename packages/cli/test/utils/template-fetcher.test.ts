...
describe("getInstallPath", () => {
  it("returns spec path for 'spec' type", () => {
    ...
    expect(result).toBe(path.join("/project", ".coding/spec"));
  });
  ...
  it("works with different cwd values", () => {
    ...
    expect(result).toBe(path.join("/home/user/my-project", ".coding/spec"));
  });
});
...
async function withGitRegistry<T>(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "coding-git-reg-"));
  ...
  try {
    ...
    execFileSync(
      ...
      [
        "-c",
        "user.name=Coding Test",
        "-c",
        "user.email=coding@example.com",
        "commit",
        "-m",
        "fixture",
      ],
...
async function withFakeGit<T>(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "coding-fake-git-"));
  ...
  it("downloads marketplace templates through the same Git backend", async () => {
    await withGitRegistry(
      ...
      async ({ registry, tmpDir }) => {
        ...
        expect(
          fs.readFileSync(
            path.join(cwd, ".coding", "spec", "rules.md"),
            "utf-8",
          ),
        ).toBe("remote rules\n");
      },
  ...
  it("uses direct download mode when index.json is absent", async () => {
    await withGitRegistry(
      ...
      async ({ registry, tmpDir }) => {
        ...
        const cwd = path.join(tmpDir, "project");
        const specDir = path.join(cwd, ".coding", "spec");
