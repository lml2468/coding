...
describe("loadHashes / saveHashes", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "coding-test-"));
    // Create .coding directory for hashes file
    fs.mkdirSync(path.join(tmpDir, ".coding"), { recursive: true });
  });
  ...
  it("loadHashes returns empty object for invalid JSON", () => {
    const hashesPath = path.join(tmpDir, ".coding", ".template-hashes.json");
...
describe("initializeHashes", () => {
  ...
  it("returns 0 when no template directories exist", () => {
    // Create .coding dir for saving hashes but no template files
    fs.mkdirSync(path.join(tmpDir, ".coding"), { recursive: true });
    const count = initializeHashes(tmpDir);
    expect(count).toBe(0);
  });

  it("hashes files in .coding/ and tracked platform paths", () => {
    // .coding/ is always walked recursively. Platform paths (.claude/, etc.)
    ...
    fs.mkdirSync(path.join(tmpDir, ".coding", "scripts"), { recursive: true });
    fs.writeFileSync(
      path.join(tmpDir, ".coding", "scripts", "task.py"),
      "print('hello')",
    );
    ...
    const hashes = loadHashes(tmpDir);
    expect(hashes).toHaveProperty(".coding/scripts/task.py");
  ...
  it("does NOT hash platform-dir files that are not in trackedPaths", () => {
    ...
    // Now: only paths coding actually wrote (recorded via writeFile) make
  ...
  it("excludes workspace and tasks directories", () => {
    fs.mkdirSync(path.join(tmpDir, ".coding", "workspace"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".coding", "workspace", "data.md"), "user data");
    fs.mkdirSync(path.join(tmpDir, ".coding", "tasks"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".coding", "tasks", "task.json"), "{}");

    const count = initializeHashes(tmpDir);
    const hashes = loadHashes(tmpDir);

    // These should be excluded
    expect(hashes).not.toHaveProperty(".coding/workspace/data.md");
    expect(hashes).not.toHaveProperty(".coding/tasks/task.json");
    expect(count).toBe(0);
  });

  it("excludes spec/ directory files from hashing", () => {
    fs.mkdirSync(path.join(tmpDir, ".coding", "spec", "guides"), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, ".coding", "spec", "frontend"), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, ".coding", "spec", "backend"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".coding", "spec", "guides", "index.md"), "# Guides");
    fs.writeFileSync(path.join(tmpDir, ".coding", "spec", "frontend", "index.md"), "# Frontend");
    fs.writeFileSync(path.join(tmpDir, ".coding", "spec", "backend", "index.md"), "# Backend");

    const count = initializeHashes(tmpDir);
    const hashes = loadHashes(tmpDir);

    // All spec/ files should be excluded
    expect(hashes).not.toHaveProperty(".coding/spec/guides/index.md");
    expect(hashes).not.toHaveProperty(".coding/spec/frontend/index.md");
    expect(hashes).not.toHaveProperty(".coding/spec/backend/index.md");
    expect(count).toBe(0);
  });
  ...
  it("collectFiles returns POSIX-normalized paths (no backslashes)", () => {
    ...
    fs.mkdirSync(path.join(tmpDir, ".coding", "scripts", "common"), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(tmpDir, ".coding", "scripts", "common", "task.py"),
      "print('x')",
    );
    ...
    expect(hashes).toHaveProperty(".coding/scripts/common/task.py");
  });
  ...
  it("does not exclude generated update-spec skills from hashing", () => {
    ...
    // `.pi/skills/coding-update-spec/`. The new model doesn't use that
    ...
    const count = initializeHashes(tmpDir, {
      trackedPaths: new Set([".pi/skills/coding-update-spec/SKILL.md"]),
    });
    const hashes = loadHashes(tmpDir);

    expect(hashes).toHaveProperty(
      ".pi/skills/coding-update-spec/SKILL.md",
    );
    expect(count).toBe(1);
  });
});
...
describe("cross-platform hash storage (POSIX keys + v2 schema)", () => {
  let tmpDir: string;
  const HASHES_REL = path.join(".coding", ".template-hashes.json");
  ...
  it("legacy file is discarded then initializeHashes regenerates v2", () => {
    ...
    fs.writeFileSync(
      path.join(tmpDir, HASHES_REL),
      JSON.stringify({ ".coding\\config.yaml": "deadbeef" }),
    );
    ...
    expect(parsed.hashes).not.toHaveProperty(".coding\\config.yaml");
    expect(parsed.hashes).not.toHaveProperty(".coding/config.yaml");
    // Newly hashed file is present with POSIX key
    expect(parsed.hashes).toHaveProperty(".coding/scripts/task.py");
  });
