...
describe("registry-config", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "coding-reg-config-"));
  ...
  it("preserves self-hosted SSH registry source strings", () => {
    ...
    writeSpecRegistryConfig(tmpDir, {
      source: "git@git.ppdaicorp.com:xionghongwei/coding-spec.git",
      template: "golang-spec",
    });
  ...
  it("does not duplicate an existing registry section", () => {
    const configPath = path.join(tmpDir, ".coding", "config.yaml");
    fs.writeFileSync(
      configPath,
