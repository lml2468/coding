...
describe("pruneOrphanManifestKeys", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "coding-prune-"));
  ...
  it("preserves every .coding/* entry regardless of platform-collect output", () => {
    const hashes = {
      ".coding/workflow.md": "h1",
      ".coding/scripts/task.py": "h2",
      ".coding/config.yaml": "h3",
    };
  ...
  it("keeps root-level AGENTS.md when it has Coding managed-block markers", () => {
  ...
  it("prunes poisoned root-level AGENTS.md when the file lacks Coding markers", () => {
  ...
  it("does NOT rewrite disk when nothing was pruned", () => {
    const hashes = { ".coding/workflow.md": "h1" };
    saveHashes(tmpDir, hashes);

    const hashFile = path.join(tmpDir, ".coding", ".template-hashes.json");
...
describe("isCwdHomedir / homedir guard helpers", () => {
  ...
  it("homedirBypassEnabled reflects CODING_ALLOW_HOMEDIR env var", () => {
    const orig = process.env.CODING_ALLOW_HOMEDIR;
    try {
      ...
      for (const value of ["0", "false", "true", ""]) {
        process.env.CODING_ALLOW_HOMEDIR = value;
        expect(homedirBypassEnabled()).toBe(false);
      }
    } finally {
      if (orig === undefined) delete process.env.CODING_ALLOW_HOMEDIR;
      else process.env.CODING_ALLOW_HOMEDIR = orig;
    }
  });
  ...
  it("homedirGuardMessage mentions the command and the bypass env var", () => {
    ...
    expect(msgInit).toContain("CODING_ALLOW_HOMEDIR=1");

    const msgUninstall = homedirGuardMessage("uninstall");
    expect(msgUninstall).toContain("uninstall");
    expect(msgUninstall).toContain("CODING_ALLOW_HOMEDIR=1");
  });
});
