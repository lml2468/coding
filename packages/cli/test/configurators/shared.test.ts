...
describe("replacePythonCommandLiterals", () => {
  ...
  it("handles multiline content with mixed shebangs and commands", () => {
    ...
    const input = [
      ...
      'exec python3 "$0" "$@"',
      "python3 ./.coding/scripts/task.py",
    ].join("\n");
    const expected = [
      "#!/usr/bin/env python3",
      "# comment about python",
      'exec python "$0" "$@"',
      "python ./.coding/scripts/task.py",
    ].join("\n");
...
describe("resolvePlaceholders", () => {
  ...
  describe("{{CMD_REF:name}}", () => {
    it("resolves with /coding: prefix (Claude)", () => {
      const result = resolvePlaceholders(
        "See {{CMD_REF:brainstorm}} for details",
        claudeCtx,
      );
      expect(result).toBe("See /coding:brainstorm for details");
    });
    ...
    it("resolves with /coding- prefix (Cursor)", () => {
      const result = resolvePlaceholders(
        "Use {{CMD_REF:finish-work}} when done",
        cursorCtx,
      );
      expect(result).toBe("Use /coding-finish-work when done");
    });
    ...
    it("handles multiple CMD_REF in one template", () => {
      ...
      expect(resolvePlaceholders(input, claudeCtx)).toBe(
        "/coding:start then /coding:brainstorm then /coding:check",
      );
    });
  ...
  describe("simple substitutions", () => {
    ...
    it("resolves {{PYTHON_CMD}} alongside context placeholders", () => {
      const result = resolvePlaceholders(
        "{{PYTHON_CMD}} ./.coding/scripts/task.py and {{CMD_REF:start}}",
        claudeCtx,
      );
      const py = process.platform === "win32" ? "python" : "python3";
      expect(result).toBe(
        `${py} ./.coding/scripts/task.py and /coding:start`,
      );
    });
  });
  ...
  describe("{{CLI_FLAG}}", () => {
    ...
    it("works alongside {{PYTHON_CMD}} in a realistic init-context invocation", () => {
      const input =
        '{{PYTHON_CMD}} ./.coding/scripts/task.py init-context "$TASK_DIR" <type> --platform {{CLI_FLAG}}';
      const py = process.platform === "win32" ? "python" : "python3";
      expect(resolvePlaceholders(input, codexCtx)).toBe(
        `${py} ./.coding/scripts/task.py init-context "$TASK_DIR" <type> --platform codex`,
      );
    });
  });
...
describe("resolvePlaceholdersNeutral", () => {
  it("renders {{CMD_REF:name}} as `name` (Coding command) — platform-neutral", () => {
    expect(
      resolvePlaceholdersNeutral("See {{CMD_REF:brainstorm}}", claudeCtx),
    ).toBe("See `brainstorm` (Coding command)");
    expect(
...
describe("resolveSkillsNeutral / resolveAllAsSkillsNeutral", () => {
  it("resolveSkillsNeutral produces byte-identical output across platforms", () => {
    const claudeSkills = resolveSkillsNeutral(AI_TOOLS["claude-code"].templateContext);
    const neutralCtxSkills = resolveSkillsNeutral(cursorCtx);
    expect(claudeSkills.length).toBe(neutralCtxSkills.length);
    for (let i = 0; i < claudeSkills.length; i++) {
      expect(claudeSkills[i].name).toBe(neutralCtxSkills[i].name);
      expect(claudeSkills[i].content).toBe(neutralCtxSkills[i].content);
    }
  });
  ...
  it("resolveSkillsNeutral renders CMD_REF without platform-specific prefix", () => {
    ...
    // Claude `/coding:name`, Cursor `/coding-name`).
    const neutral = resolveSkillsNeutral(AI_TOOLS["claude-code"].templateContext);
    const cmdRefNames = [
      "start",
      "brainstorm",
      "check",
  ...
  it("resolveAllAsSkillsNeutral keeps shared common skills byte-identical to resolveSkillsNeutral", () => {
    const all = resolveAllAsSkillsNeutral(AI_TOOLS["claude-code"].templateContext);
    const commonSkills = resolveSkillsNeutral(AI_TOOLS["claude-code"].templateContext);
...
describe("wrapWithOmpFrontmatter", () => {
  ...
  it("strips coding- prefix before looking up description", () => {
    const content = "# Continue Current Task\n\nBody text.";
    const result = wrapWithOmpFrontmatter("coding-continue", content);
    expect(result).toMatch(/^---\ndescription: /);
