...
describe("ALL_MANAGED_DIRS", () => {
  it("starts with .coding", () => {
    expect(ALL_MANAGED_DIRS[0]).toBe(".coding");
  });

  it("contains .coding plus all managed dirs", () => {
...
describe("isManagedPath", () => {
  ...
  it("matches platform config sub-paths", () => {
    ...
    expect(isManagedPath(".claude/skills/coding-start/SKILL.md")).toBe(true);
  });
  ...
  it("matches exact managed directory names", () => {
    ...
    expect(isManagedPath(".coding")).toBe(true);
  });

  // Positive: .coding hardcoded paths
  it("matches .coding sub-paths", () => {
    expect(isManagedPath(".coding/spec")).toBe(true);
    expect(isManagedPath(".coding/tasks/some-task")).toBe(true);
  });
  ...
  it("rejects prefix-similar non-sub-paths", () => {
    ...
    expect(isManagedPath(".coding-old")).toBe(false);
  });
  ...
  it("rejects path traversal", () => {
    ...
    expect(isManagedPath("../.coding/spec")).toBe(false);
  });
...
describe("isManagedRootDir", () => {
  ...
  it("matches .coding", () => {
    expect(isManagedRootDir(".coding")).toBe(true);
  });

  it("rejects sub-paths (not a root dir)", () => {
    expect(isManagedRootDir(".claude/commands")).toBe(false);
    expect(isManagedRootDir(".coding/spec")).toBe(false);
  });
...
describe("collectPlatformTemplates", () => {
  ...
  it("tracks bundled built-in skill files for every skill-writing platform", () => {
    for (const [id, skillRoot] of Object.entries(SKILL_ROOTS)) {
      ...
      expect(result?.has(`${skillRoot}/coding-meta/SKILL.md`)).toBe(true);
      expect(
        result?.has(
          `${skillRoot}/coding-meta/references/local-architecture/overview.md`,
        ),
      ).toBe(true);
      expect(result?.has(`${skillRoot}/coding-spec-bootstrap/SKILL.md`)).toBe(
        true,
      );
      expect(
        result?.has(
          `${skillRoot}/coding-spec-bootstrap/references/spec-writing.md`,
        ),
      ).toBe(true);
    }
  });
