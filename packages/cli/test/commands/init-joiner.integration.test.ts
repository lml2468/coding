 ...
 *   no .coding/                        → creator bootstrap task
 *   .coding/ exists, .developer missing → joiner onboarding task
...
describe("init() joiner onboarding", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "coding-joiner-int-"));
  ...
  /**
   * Helper: simulate a fresh clone of an existing Coding project — `.coding/`
  ...
  /** Helper: simulate same-dev re-init — both `.coding/` and `.developer` exist */
  ...
  it("#2 existing .coding/ + no .developer → joiner onboarding task created", async () => {
    simulateExistingCheckout();
    ...
    expect(prd).toContain(".coding/spec/");
    ...
    expect(prd).toContain(
      `${expectedPythonCmd} ./.coding/scripts/task.py list --assignee bob`,
    );
    expect(prd).toContain(`${expectedPythonCmd} ./.coding/scripts/task.py finish`);
    expect(prd).toContain(
      `${expectedPythonCmd} ./.coding/scripts/task.py archive 00-join-bob`,
    );
  ...
  it("#2b issue #204: existing .coding/ but tasks/ empty → bootstrap fallback (--yes alone, no --force)", async () => {
    // Mirrors the exact reproduction in issue #204: first run aborted partway
    // after writing the .coding/ skeleton but before creating bootstrap;
  ...
  it("#3 existing .coding/ + .developer → no task created", async () => {
  ...
  it("#6 joiner creation failure surfaces as warning, init does not crash", async () => {
    ...
    // thwarted by making .coding/tasks read-only right before dispatch,
  ...
  // Tests #7/#8 cover the handleReinit path — the default flow when .coding/
  ...
  it("#7 handleReinit path: existing .coding/ + no .developer → joiner task created", async () => {
    simulateExistingCheckout();
  ...
  it("#8 handleReinit path: existing .coding/ + .developer → no task created", async () => {
