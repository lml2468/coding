...
describe("init + uninstall: manifest accuracy + homedir guard", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "coding-overdelete-"));
    ...
    delete process.env.CODING_ALLOW_HOMEDIR;
  });
  ...
  // ----- R1.6: Coding-written AGENTS.md with user content outside the block -----

  it("#R1.6 uninstall strips only the Coding block from a tracked AGENTS.md, preserving user content outside it", async () => {
    // Coding writes AGENTS.md (so it's manifest-tracked, unlike the
    // skip-existing cases above). The user then adds their own instructions
    // OUTSIDE the <!-- CODING:START/END --> block — a supported layout that
    // update.ts explicitly preserves. Uninstall must not unlink the whole
    // file and take the user content with it.
    await init({ yes: true, claude: true, force: true });
    const agentsPath = path.join(tmpDir, "AGENTS.md");
    // Sanity: Coding tracked it and wrote the managed block.
    expect(loadHashes(tmpDir)).toHaveProperty("AGENTS.md");
    const written = fs.readFileSync(agentsPath, "utf-8");
    expect(written).toContain("<!-- CODING:START -->");
    ...
    expect(after).not.toContain("<!-- CODING:START -->");
    expect(after).not.toContain("<!-- CODING:END -->");
    expect(after).toContain("## My project rules");
    expect(after).toContain("Always rebase.");
  });

  it("#R1.7 uninstall still deletes a tracked AGENTS.md that is only the Coding block", async () => {
  ...
  it("#R3.2 uninstall self-heals + preserves user file even without prior update", async () => {
    ...
    // and runs `coding uninstall` directly. Prune must fire before plan
  ...
  it("#R3.3 prune keeps migration-referenced paths even if not in collectTemplates", async () => {
    ...
    // CAN assert the prune behavior preserves .coding/ entries which is the
    ...
    const hashes = loadHashes(tmpDir);
    hashes[".coding/workflow.md"] = "ok";
    saveHashes(tmpDir, hashes);

    await update({});

    // .coding/* entries are kept.
    expect(loadHashes(tmpDir)).toHaveProperty(".coding/workflow.md");
  });
  ...
  it("#R2.1 init refuses to run when cwd === $HOME", async () => {
    ...
    try {
      ...
      // No .coding dir was created.
      expect(fs.existsSync(path.join(fakeHome, ".coding"))).toBe(false);
    } finally {
  ...
  it("#R2.2 uninstall refuses to run when cwd === $HOME", async () => {
    // Set up a valid coding project, then pretend its cwd is the homedir.
    await init({ yes: true, claude: true, force: true });
    ...
    expect(fs.existsSync(path.join(tmpDir, ".coding"))).toBe(true);
  });

  it("#R2.3 CODING_ALLOW_HOMEDIR=1 bypasses the guard for init", async () => {
    const fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), "fake-home-"));
    try {
      vi.spyOn(process, "cwd").mockReturnValue(fakeHome);
      process.env.CODING_ALLOW_HOMEDIR = "1";

      await withFakeHome(fakeHome, async () => {
        await init({ yes: true, claude: true, force: true });
      });

      expect(fs.existsSync(path.join(fakeHome, ".coding"))).toBe(true);
    } finally {
      fs.rmSync(fakeHome, { recursive: true, force: true });
    }
  });
  ...
  it("#R2.4 subdirectories of $HOME are NOT blocked", async () => {
    ...
    try {
      ...
      expect(fs.existsSync(path.join(subDir, ".coding"))).toBe(true);
    } finally {
      fs.rmSync(fakeHome, { recursive: true, force: true });
    }
  });
