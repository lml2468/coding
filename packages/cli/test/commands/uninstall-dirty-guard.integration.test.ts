 ...
 *
 * `coding uninstall` deletes the whole .coding/ tree — including
...
describe.skipIf(!canRun)("uninstall uncommitted-data guard", () => {
  ...
  beforeEach(async () => {
    tmpDir = fs.realpathSync(
      fs.mkdtempSync(path.join(os.tmpdir(), "coding-dirty-")),
    );
    ...
    delete process.env.CODING_ALLOW_DIRTY_UNINSTALL;
  ...
  it("detects a newly added spec file once the tree is committed", () => {
    ...
    // (an entirely-untracked .coding collapses to the dir name instead).
    git(tmpDir, "add", "-A");
    git(tmpDir, "commit", "-q", "-m", "coding");

    const specFile = path.join(tmpDir, ".coding", "spec", "my-rules.md");
    fs.mkdirSync(path.dirname(specFile), { recursive: true });
    fs.writeFileSync(specFile, "my custom spec");

    const dirty = collectUncommittedCodingData(tmpDir);
    expect(dirty.some((p) => p.includes("spec/my-rules.md"))).toBe(true);
  });

  it("reports nothing once the .coding tree is committed", () => {
    git(tmpDir, "add", "-A");
    git(tmpDir, "commit", "-q", "-m", "coding");
    expect(collectUncommittedCodingData(tmpDir)).toEqual([]);
  });

  it("refuses --yes uninstall while user data is uncommitted, leaving .coding intact", async () => {
    const specFile = path.join(tmpDir, ".coding", "spec", "my-rules.md");
    fs.mkdirSync(path.dirname(specFile), { recursive: true });
  ...
  it("CODING_ALLOW_DIRTY_UNINSTALL=1 overrides the guard", async () => {
    const specFile = path.join(tmpDir, ".coding", "spec", "my-rules.md");
    fs.mkdirSync(path.dirname(specFile), { recursive: true });
    fs.writeFileSync(specFile, "unsaved work");
    process.env.CODING_ALLOW_DIRTY_UNINSTALL = "1";
