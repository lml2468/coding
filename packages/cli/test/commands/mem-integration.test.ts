...
const { fakeHome } = vi.hoisted(() => {
  ...
  const fakeHome = f.mkdtempSync(p.join(o.tmpdir(), "coding-mem-int-"));
  return { fakeHome };
});
...
describe("runMem subcommand integration", () => {
        ...
        message: {
          ...
          content: [
            { type: "text", text: "creating task" },
            {
              ...
              input: {
                command:
                  "python3 ./.coding/scripts/task.py create --slug demo",
              },
            },
          ],
        },
        ...
        message: {
          ...
          content: [
            { type: "text", text: "starting" },
            {
              ...
              input: {
                command:
                  "python3 ./.coding/scripts/task.py start .coding/tasks/demo",
              },
            },
          ],
        },
  ...
  it("help command prints usage", () => {
    runMem(["help"]);
    const joined = logs.join("\n");
    expect(joined).toContain("coding mem");
    expect(joined).toContain("claude|all");
  });
