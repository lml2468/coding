 ...
 *
 * These test cleanupEmptyDirs, loadUpdateSkipPaths, and shouldExcludeFromBackup
...
describe("cleanupEmptyDirs", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "coding-cleanup-"));
  });
  ...
  it("[CR#1] does not delete .coding root even if empty", () => {
    fs.mkdirSync(path.join(tmpDir, ".coding"), { recursive: true });
    cleanupEmptyDirs(tmpDir, ".coding");
    expect(fs.existsSync(path.join(tmpDir, ".coding"))).toBe(true);
  });

  it("recursively cleans parent directories but stops at root", () => {
    // Create .coding/scripts/multi_agent/ (all empty)
    fs.mkdirSync(path.join(tmpDir, ".coding", "scripts", "multi_agent"), {
      recursive: true,
    });
    cleanupEmptyDirs(tmpDir, ".coding/scripts/multi_agent");

    // multi_agent and scripts should be removed (both empty)
    expect(
      fs.existsSync(
        path.join(tmpDir, ".coding", "scripts", "multi_agent"),
      ),
    ).toBe(false);
    expect(
      fs.existsSync(path.join(tmpDir, ".coding", "scripts")),
    ).toBe(false);
    // .coding root must survive
    expect(fs.existsSync(path.join(tmpDir, ".coding"))).toBe(true);
  });
...
describe("loadUpdateSkipPaths", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "coding-skip-"));
    fs.mkdirSync(path.join(tmpDir, ".coding"), { recursive: true });
  });
...
describe("shouldExcludeFromBackup", () => {
  ...
  it.each([
    ...
    ".coding/.backup-2026-04-22T10-24-27/.opencode/node_modules/zod/index.js",
  ])("excludes dependency tree %s", (p) => {
    expect(shouldExcludeFromBackup(p)).toBe(true);
  });

  it.each([
    ".coding/workspace/developer/journal-1.md",
    ".coding/tasks/04-17-foo/prd.md",
    ".coding/spec/cli/backend/index.md",
    ".coding/backlog/idea.md",
    ".coding/agent-traces/trace.jsonl",
  ...
  it("excludes previous backups", () => {
    expect(
      shouldExcludeFromBackup(".coding/.backup-2026-04-20T01-00-00/x"),
    ).toBe(true);
  });

  it.each([
    ".claude/commands/coding/continue.md",
    ".claude/skills/coding-check/SKILL.md",
    ".coding/workflow.md",
    ".coding/scripts/get_context.py",
    ".agents/skills/coding-check/SKILL.md",
  ...
  // after normalization, otherwise Coding's native worktree protection
  ...
  // `coding update --migrate`, late April 2026).
  it.each([
    ".claude\\worktrees\\feat-x\\src\\main.ts",
    ".coding\\tasks\\04-17-foo\\prd.md",
    ".coding\\workspace\\dev\\journal-1.md",
