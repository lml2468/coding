...
describe("requireSupportedPython", () => {
  ...
  it("throws when the detected version is below the supported floor", () => {
    ...
    expect(() => requireSupportedPython("python3")).toThrow(
      'Python 3.8.18 detected via "python3", but Coding init requires Python ≥ 3.9.',
    );
  });
  ...
  it("throws when the command is missing", () => {
    ...
    expect(() => requireSupportedPython("python")).toThrow(
      'Python command "python" not found. Coding init requires Python ≥ 3.9.',
    );
  });
  ...
  it("skips the probe entirely when CODING_SKIP_PYTHON_CHECK=1", () => {
    const prev = process.env.CODING_SKIP_PYTHON_CHECK;
    process.env.CODING_SKIP_PYTHON_CHECK = "1";
    try {
      // execSync should not be called at all
      const result = requireSupportedPython("python3");
      expect(result).toBe("version check skipped (CODING_SKIP_PYTHON_CHECK=1)");
      expect(execSync).not.toHaveBeenCalled();
    } finally {
      if (prev === undefined) {
        delete process.env.CODING_SKIP_PYTHON_CHECK;
      } else {
        process.env.CODING_SKIP_PYTHON_CHECK = prev;
      }
    }
  });
});
...
describe("resolveSupportedPython", () => {
  beforeEach(() => {
    vi.mocked(execSync).mockReset();
    resetResolvedPythonCommand();
    delete process.env.CODING_PYTHON_CMD;
    delete process.env.CODING_SKIP_PYTHON_CHECK;
  });
  ...
  it("honors CODING_PYTHON_CMD as an explicit override (no probe)", () => {
    process.env.CODING_PYTHON_CMD = "py -3.12";
  ...
  it("honors CODING_SKIP_PYTHON_CHECK=1 as last-resort escape hatch", () => {
    process.env.CODING_SKIP_PYTHON_CHECK = "1";
