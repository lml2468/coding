...
describe("uninstall() integration", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "coding-uninstall-int-"));
  ...
  it("#1 friendly exit when .coding/ is missing", async () => {
  ...
  it("#2 errors when manifest is missing but .coding/ exists", async () => {
  ...
  it("#3 init → uninstall → project is clean", async () => {
    ...
    // .coding/ should be gone.
    expect(fs.existsSync(path.join(tmpDir, ".coding"))).toBe(false);
    ...
    // package.json) may legitimately remain when the coding template
    // shipped non-coding fields too (e.g. .claude/settings.json's `env`
  ...
  it("#6 user-modified coding file is still deleted (manifest defines scope)", async () => {
    await init({ yes: true, claude: true, force: true });

    // Pick any opaque manifest-tracked file under .claude/ and overwrite it.
    // Structured config files (settings.json) may survive scrubbing, so pick
    // a non-structured entry.
    const hashesBefore = loadHashes(tmpDir);
    const claudeTrackedPath = Object.keys(hashesBefore).find(
      (p) => p.startsWith(".claude/") && !p.endsWith("/settings.json"),
    );
    if (!claudeTrackedPath) {
      throw new Error(
        "Test fixture: expected at least one opaque .claude/ entry in manifest",
      );
    }
  ...
  it("#8 .claude/settings.json with extra user fields keeps user fields, strips coding hooks", async () => {
    ...
    // hook entry alongside the coding ones.
    ...
    // .coding/ is gone, but settings.json should remain (had user fields).
    if (fs.existsSync(settingsPath)) {
      ...
      // User hook (if it was inserted) should still be present, coding ones gone.
