...
describe("upgrade command", () => {
  ...
  it("builds POSIX npm global install command without shell", () => {
    expect(
      ...
      command: "npm",
      args: ["install", "-g", "@limenglin/coding@beta"],
      spawnOptions: { stdio: "inherit", shell: false },
      displayCommand: "npm install -g @limenglin/coding@beta",
      target: "@limenglin/coding@beta",
      tag: "beta",
      binaryCheckCommand: "which coding",
    });
  });
  ...
  it("builds Windows command through cmd.exe", () => {
    expect(
      ...
      command: "cmd.exe",
      args: ["/d", "/s", "/c", "npm install -g @limenglin/coding@beta"],
      spawnOptions: { stdio: "inherit", shell: false },
      displayCommand: "npm install -g @limenglin/coding@beta",
      target: "@limenglin/coding@beta",
      tag: "beta",
      binaryCheckCommand: "where coding",
    });
  });
  ...
  it("dry-run does not execute npm", async () => {
    ...
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining("Run: npm install -g @limenglin/coding@latest"),
    );
  ...
  it("executes npm install for real upgrades", async () => {
    ...
    expect(runner).toHaveBeenCalledWith(
      "npm",
      ["install", "-g", "@limenglin/coding@latest"],
      { stdio: "inherit", shell: false },
    );
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining("coding --version"),
    );
    expect(log).toHaveBeenCalledWith(expect.stringContaining("which coding"));
  ...
  it("fails when npm exits non-zero", async () => {
    ...
    await expect(upgrade({ tag: "latest" }, runner)).rejects.toThrow(
      /npm install failed with exit code 1\.[\s\S]*Troubleshooting:[\s\S]*Manual command: npm install -g @limenglin\/coding@latest[\s\S]*npm config get prefix[\s\S]*which coding/,
    );
