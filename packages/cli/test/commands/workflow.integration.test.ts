/**
 * Integration tests for `coding workflow` and the init/update hash boundary
 * for non-native workflow selection.
 *
 * Coverage:
 * - `coding workflow --template native`: writes bundled content, keeps hash.
 * - `coding workflow --template tdd`: writes marketplace content, removes hash.
 * - `coding init --workflow tdd`: marketplace content is written, hash removed.
 * - `coding update` after switch to tdd does NOT silently restore native.
...
describe("coding workflow integration", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "coding-workflow-int-"));
  ...
  it("coding workflow --template native refreshes hash after switching from tdd", async () => {
    stubMarketplaceFetch();
    await init({ yes: true, workflow: "tdd" } as Record<string, unknown>);
    expect(
  ...
  it("coding workflow --template tdd writes marketplace content and removes the hash", async () => {
    stubMarketplaceFetch();
    await init({ yes: true });
    expect(loadHashes(tmpDir)[PATHS.WORKFLOW_GUIDE_FILE]).toBeTruthy();
  ...
  it("coding update after switching to tdd does not silently restore native workflow", async () => {
    stubMarketplaceFetch();
    await init({ yes: true });
    await runWorkflowCommand({ template: "tdd" });
