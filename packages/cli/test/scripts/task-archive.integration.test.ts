 ...
 * `src/templates/coding/scripts/common/task_store.py`; this test stamps
...
function runArchive(repo: string, taskName: string): void {
  const r = spawnSync(
    "python3",
    [".coding/scripts/task.py", "archive", taskName],
...
describe.skipIf(!hasPython())(
  ...
  () => {
    ...
    beforeEach(() => {
      tmp = fs.mkdtempSync(path.join(os.tmpdir(), "coding-archive-test-"));
      setupRepo(tmp);
    });
    ...
    it("does not bundle dirty changes from other task dirs (scope-creep fix)", () => {
      ...
      expect(status).toMatch(/M\s+\.coding\/tasks\/task-b\/prd\.md/);
    });
    ...
    it(
      ...
      () => {
        ...
        expect(
          deletes.every((p) => p.startsWith(".coding/tasks/big/")),
        ).toBe(true);
      },
    ...
    it("refuses to archive a mistyped name that resolves to a real source dir", () => {
      ...
      // dir into .coding/tasks/archive/.
      const r = spawnSync(
        "python3",
        [".coding/scripts/task.py", "archive", "src"],
      ...
      expect(
        fs.existsSync(
          path.join(tmp, ".coding", "tasks", "archive"),
        ),
      ).toBe(false);
    });
    ...
    it("fails when archive auto-commit cannot record tracked source deletes", () => {
      ...
      const r = spawnSync(
        "python3",
        [".coding/scripts/task.py", "archive", "tracked"],
        { cwd: tmp, encoding: "utf-8" },
      );
      ...
      const status = git(tmp, "status", "--porcelain");
      expect(status).toContain(".coding/tasks/tracked/");
      expect(status).toContain(".coding/tasks/archive/");
    });
  },
);
