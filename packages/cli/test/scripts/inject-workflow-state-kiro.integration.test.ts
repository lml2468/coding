...
function setupRepo(tmp: string): void {
  fs.mkdirSync(path.join(tmp, ".coding", "scripts"), { recursive: true });
  fs.cpSync(TEMPLATE_SCRIPTS, path.join(tmp, ".coding", "scripts"), {
    recursive: true,
  });
  // workflow.md with a no_task breadcrumb so the body is deterministic.
  fs.writeFileSync(
    path.join(tmp, ".coding", "workflow.md"),
    [
      "# Workflow",
      "",
      "## Phase Index",
      ...
      "No active task. Classify the turn before creating a Coding task.",
      "[/workflow-state:no_task]",
      "",
      "## Phase 1: Plan",
      "",
...
describeFn("Kiro hook output branch", () => {
  ...
  beforeEach(() => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), "coding-kiro-hook-"));
    setupRepo(tmp);
  });
