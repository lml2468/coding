 ...
 * `src/templates/coding/scripts/add_session.py`; this test stamps the real
...
function setupRepo(tmp: string): void {
  ...
  fs.writeFileSync(
    path.join(tmp, ".coding", "config.yaml"),
  ...
  const r = spawnSync(
    "python3",
    [".coding/scripts/init_developer.py", DEVELOPER],
...
function makeTask(
  ...
  const dir = path.join(repo, ".coding", "tasks", name);
...
function setCurrentTask(repo: string, taskName: string): void {
  const sessionsDir = path.join(repo, ".coding", ".runtime", "sessions");
  fs.mkdirSync(sessionsDir, { recursive: true });
  fs.writeFileSync(
    path.join(sessionsDir, "session.json"),
    JSON.stringify({
      current_task: `.coding/tasks/${taskName}`,
      platform: "session",
    }) + "\n",
  );
}
...
function runAddSession(repo: string, title: string): void {
  const r = spawnSync(
    "python3",
    [".coding/scripts/add_session.py", "--title", title],
...
describe.skipIf(!hasPython())("add_session.py auto-commit", () => {
  ...
  beforeEach(() => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), "coding-session-test-"));
    setupRepo(tmp);
  });
  ...
  it("does not bundle dirty changes from other task dirs (scope-creep fix)", () => {
    ...
    fs.appendFileSync(
      path.join(tmp, ".coding", "tasks", "task-b", "prd.md"),
    ...
    expect(status).toMatch(/\.coding\/tasks\/task-b\/prd\.md/);
  });
  ...
  it("uses explicit fallback text instead of journal placeholders", () => {
    ...
    const journal = fs.readFileSync(
      path.join(tmp, ".coding", "workspace", DEVELOPER, "journal-1.md"),
      "utf-8",
    );
  ...
  it("falls back to the current checkout branch when task.json branch is stale", () => {
    ...
    const index = fs.readFileSync(
      path.join(tmp, ".coding", "workspace", DEVELOPER, "index.md"),
      "utf-8",
    );
  ...
  it("does not wide-scan task dirs when the current task is unresolvable (>=2 sessions)", () => {
    ...
    setCurrentTask(tmp, "task-a");
    const sessionsDir = path.join(tmp, ".coding", ".runtime", "sessions");
    fs.writeFileSync(
      path.join(sessionsDir, "session2.json"),
      JSON.stringify({
        current_task: ".coding/tasks/task-b",
        platform: "session",
      }) + "\n",
    );
    git(tmp, "add", "-A");
    ...
    fs.appendFileSync(
      path.join(tmp, ".coding", "tasks", "task-a", "prd.md"),
      "DIRTY A\n",
    );
    fs.appendFileSync(
      path.join(tmp, ".coding", "tasks", "task-b", "prd.md"),
    ...
    const status = git(tmp, "status", "--porcelain");
    expect(status).toMatch(/\.coding\/tasks\/task-a\/prd\.md/);
    expect(status).toMatch(/\.coding\/tasks\/task-b\/prd\.md/);
  });
});
