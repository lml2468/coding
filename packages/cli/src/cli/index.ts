...
/**
 * Check if a Coding update is available (compare project version with CLI version)
 */
function checkForUpdates(cwd: string): void {
  ...
  if (comparison > 0) {
    ...
    console.log(
      chalk.yellow(
        `\n⚠️  Coding update available: ${projectVersion} → ${cliVersion}`,
      ),
    );
    console.log(chalk.gray(`   Run: coding update\n`));
    ...
    console.log(chalk.gray(`   Run: coding upgrade\n`));
  }
}

// Check for updates at CLI startup (only if .coding exists)
...
program
  .command("init")
  .description("Initialize coding in the current project")
  .option("--claude", "Include Claude Code commands")
  .option(
    "--with-statusline",
    "Install the Coding statusLine for Claude Code (off by default)",
  )
  ...
  .option(
    "--workflow <id>",
    "Workflow template id for .coding/workflow.md (default: native; e.g., tdd)",
  )
  .option(
  ...
  .action(async (options: Record<string, unknown>) => {
    try {
      ...
      if (process.env.DEBUG || process.env.CODING_DEBUG) {
...
program
  .command("update")
  .description("Update coding configuration and commands to latest version")
...
program
  .command("upgrade")
  .description("Upgrade the global Coding CLI package")
  .option(
    "--tag <tag>",
...
program
  .command("uninstall")
  .description(
    "Remove all coding files (managed platform files + .coding/) from this project",
  )
...
program
  .command("workflow")
  .description(
    "List or switch the project's .coding/workflow.md template (native, tdd, or marketplace)",
  )
  .option(
    "-t, --template <id>",
  ...
  .option(
    "-n, --create-new",
    "Write .coding/workflow.md.new instead of replacing the active workflow",
  )
  .action(async (options: Record<string, unknown>) => {
    try {
