...
import { copyCodingDir } from "../templates/extract.js";

// Import coding templates (generic, not project-specific)
import {
  workflowMdTemplate,
  configYamlTemplate,
  gitignoreTemplate,
...
export interface WorkflowOptions {
  ...
  /**
   * Optional override for `.coding/workflow.md` content. When omitted the
   ...
   * Caller is still responsible for removing the `.coding/workflow.md` hash
 ...
 *
 * This function creates the .coding/ directory structure by:
...
export async function createWorkflowStructure(
  ...
  // Create base .coding directory
  ensureDir(path.join(cwd, DIR_NAMES.WORKFLOW));

  // Copy scripts/ directory from templates
  await copyCodingDir("scripts", path.join(cwd, PATHS.SCRIPTS), {
    executable: true,
  });
