...
/**
 * Per-platform configure options threaded from `coding init` flags.
 * Defined here (not in index.ts) so configurators can reference it without
 * a circular import.
 */
export interface PlatformConfigureOptions {
  /**
   * Claude Code only: install the opt-in Coding statusLine
   * (`coding init --with-statusline`). Off by default — see
 ...
 * rendered in a platform-neutral form (`` `name` (Coding command) ``)
 ...
 * of which Coding configurator wrote them — eliminating the
 ...
 * neutral. Codex-only skill files (e.g. `coding-continue/SKILL.md`,
 * `coding-finish-work/SKILL.md` written via `resolveAllAsSkillsNeutral`) DO
...
export function resolvePlaceholdersNeutral(
  ...
  result = result.replace(
    RE_CMD_REF,
    (_match, name: string) => `\`${name}\` (Coding command)`,
  );
  result = result.replace(RE_EXECUTOR_AI, context.executorAI);
  result = result.replace(RE_USER_ACTION_LABEL, context.userActionLabel);
  result = result.replace(RE_CLI_FLAG, context.cliFlag);
...
export function wrapWithSkillFrontmatter(
  ...
  // Look up description by base name (without coding- prefix)
  const baseName = name.replace(/^coding-/, "");
...
const COMMAND_DESCRIPTIONS: Record<string, string> = {
  start: "Initialize a Coding development session.",
...
export interface ResolvedSkillFile {
  /** POSIX path relative to the skills root, e.g. "coding-meta/SKILL.md" */
  relativePath: string;
  content: string;
}
 ...
 * user-invocable `coding-start` skill / `start.md` command as fallback.
...
/**
 * Resolve ALL templates as skills with coding- prefix.
...
/**
 * Resolve the auto-triggered skill templates from `common/skills/` with coding- prefix + SKILL.md frontmatter.
...
/** Build the standard "load Coding context first" prelude block. */
export function buildPullBasedPrelude(agentType: SubAgentType): string {
  ...
  return replacePythonCommandLiterals(`## Required: Load Coding Context First
...
1. **Look at the dispatch prompt** you received from the main agent. If its first line is \`Active task: <path>\` (e.g. \`Active task: .coding/tasks/04-17-foo\`), use that path. The main agent is required to include this line on class-2 platforms.
2. **Run** \`python3 ./.coding/scripts/task.py current --source\` and read the \`Current task:\` line.
...
/** Best-effort detect agent type from filename ("coding-implement.md" → "implement").
...
export function detectSubAgentType(name: string): SubAgentType | null {
  ...
  if (base === "coding-implement" || base === "coding-check") {
    return base === "coding-implement" ? "implement" : "check";
  }
  return null;
}
...
function mapLegacyToolToCopilot(tool: string): string[] {
  switch (tool) {
    ...
    case "Bash":
      return ["execute"];
    // Generic MCP wildcard — used by coding-research to opt into "any MCP
