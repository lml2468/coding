 ...
 * Byte-parity matters: `coding update` re-derives the expected settings.json
 ...
 * - commands/coding/ — start + finish-work as slash commands
 * - skills/coding-{name}/SKILL.md — auto-triggered skills from `common/skills/`
 * - with `withStatusline`: opt-in statusline.py hook + `statusLine` settings
 *   entry (off by default; `coding init --with-statusline`)
 */
export async function configureClaude(
  ...
  // not in collectTemplates, so `coding update` never force-installs it)
  if (withStatusline) {
    await writeFile(
  ...
  const commandsDir = path.join(destPath, "commands", "coding");
