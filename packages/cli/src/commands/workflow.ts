/**
 * `coding workflow` command — list and switch the active `.coding/workflow.md`.
 *
 * Behavior contracts:
 *
 * - Hash boundary: after writing native content, refresh the
 *   `.coding/workflow.md` entry in `.template-hashes.json`. After writing
 *   any non-native content, remove that entry. This prevents `coding update`
 ...
 *
 * - `--create-new`: never touches `.coding/workflow.md`; writes
 *   `.coding/workflow.md.new` and leaves the hash file alone.
 */
  ...
  const { action } = await inquirer.prompt<{ action: string }>([
    {
      ...
      name: "action",
      message:
        "Your .coding/workflow.md has local edits. What do you want to do?",
      choices: [
        { name: "Overwrite (replace local edits)", value: "overwrite" },
        {
          name: "Write to .coding/workflow.md.new and keep current",
          value: "create-new",
        },
...
function applyHashContract(cwd: string, templateId: string): void {
  ...
  if (templateId === NATIVE_WORKFLOW_ID) {
    ...
    // so `coding update` treats it as modified and does not silently restore
    // native bytes.
...
export async function runWorkflowCommand(
  ...
  if (!fs.existsSync(path.join(cwd, DIR_NAMES.WORKFLOW))) {
    throw new WorkflowCommandError(
      "No .coding/ directory found. Run `coding init` first.",
    );
  }
