...
export function requireSupportedPython(command: string): string {
  ...
  if (process.env.CODING_SKIP_PYTHON_CHECK === "1") {
    return `version check skipped (CODING_SKIP_PYTHON_CHECK=1)`;
  }
  ...
  if (!versionOutput) {
    throw new Error(
      `Python command "${command}" not found. Coding init requires Python ≥ 3.9.`,
    );
  }

  if (!isSupportedPythonVersion(versionOutput)) {
    throw new Error(
      `${versionOutput} detected via "${command}", but Coding init requires Python ≥ 3.9.`,
    );
  }
 ...
 * `python`) had `coding init` fail outright.
 *
 ...
 *
 * Honors `CODING_PYTHON_CMD` (explicit override, no probe) and
 * `CODING_SKIP_PYTHON_CHECK=1` (skip probe, trust platform default).
 *
...
export function resolveSupportedPython(): {
  ...
  const override = process.env.CODING_PYTHON_CMD?.trim();
  if (override) {
    setResolvedPythonCommand(override);
    return { command: override, version: "set via CODING_PYTHON_CMD" };
  }
  ...
  if (process.env.CODING_SKIP_PYTHON_CHECK === "1") {
    ...
    return {
      command: fallback,
      version: "version check skipped (CODING_SKIP_PYTHON_CHECK=1)",
    };
  }
  ...
  const installHint = isWindows
    ? `Install Python ≥ 3.9 from https://www.python.org/downloads/windows/ — make sure ` +
      ...
      `installed under a different name, set CODING_PYTHON_CMD=<your-cmd> ` +
      `before re-running init (e.g. \`set CODING_PYTHON_CMD=py -3\`).`
    : `Install Python ≥ 3.9 from https://www.python.org/downloads/ or via your ` +
      `package manager. Or set CODING_PYTHON_CMD=<your-cmd> before re-running.`;

  throw new Error(
    `No supported Python command found. Tried: ${candidates.join(", ")}.\n` +
      `Probe results:\n  ${probeFailures.join("\n  ")}\n\n` +
      `Coding init requires Python ≥ 3.9. ${installHint}\n` +
      `Last-resort escape hatch: set CODING_SKIP_PYTHON_CHECK=1 to skip the probe entirely.`,
  );
}
...
function logPythonAdaptationNotice(command: string): void {
  ...
  console.log(
    chalk.blue(
      `📌 ${osName} detected: Coding rendered Python commands as "${command}" in generated hooks, settings, and help text`,
    ),
  );
}
...
function getBootstrapRelatedFiles(
  ...
  if (packages && packages.length > 0) {
    return packages.map((pkg) => `.coding/spec/${sanitizePkgName(pkg.name)}/`);
  }
  if (projectType === "frontend") {
    return [".coding/spec/frontend/"];
  }
  if (projectType === "backend") {
    return [".coding/spec/backend/"];
  }
  return [".coding/spec/backend/", ".coding/spec/frontend/"];
}
...
|------|------------------|
| \`.coding/spec/backend/directory-structure.md\` | Where different file types go (routes, services, utils) |
| \`.coding/spec/backend/database-guidelines.md\` | ORM, migrations, query patterns, naming conventions |
| \`.coding/spec/backend/error-handling.md\` | How errors are caught, logged, and returned |
| \`.coding/spec/backend/logging-guidelines.md\` | Log levels, format, what to log |
| \`.coding/spec/backend/quality-guidelines.md\` | Code review standards, testing requirements |
`;
...
| File | What to document |
|------|------------------|
| \`.coding/spec/frontend/directory-structure.md\` | Component/page/hook organization |
| \`.coding/spec/frontend/component-guidelines.md\` | Component patterns, props conventions |
| \`.coding/spec/frontend/hook-guidelines.md\` | Custom hook naming, patterns |
| \`.coding/spec/frontend/state-management.md\` | State library, patterns, what goes where |
| \`.coding/spec/frontend/type-safety.md\` | TypeScript conventions, type organization |
| \`.coding/spec/frontend/quality-guidelines.md\` | Linting, testing, accessibility |
`;
...
\`\`\`bash
${pythonCmd} ./.coding/scripts/task.py finish
${pythonCmd} ./.coding/scripts/task.py archive 00-bootstrap-guidelines
\`\`\`
...
`;
  ...
  if (packages && packages.length > 0) {
    ...
    for (const pkg of packages) {
      ...
      if (pkgType !== "frontend") {
        content += `\n- Backend guidelines: \`.coding/spec/${specName}/backend/\`\n`;
      }
      if (pkgType !== "backend") {
        content += `\n- Frontend guidelines: \`.coding/spec/${specName}/frontend/\`\n`;
      }
    }
...
function getBootstrapTaskJson(
  ...
  return emptyTaskJson({
    ...
    createdAt: today,
    relatedFiles,
    notes: `First-time setup task created by coding init (${projectType} project)`,
  });
}
...
function getJoinerTaskJson(developer: string, taskName: string): TaskJson {
  ...
  return emptyTaskJson({
    id: taskName,
    name: taskName,
    title: `Joining: Onboard to this Coding project (${developer})`,
    description:
      "Onboard a new developer to an existing Coding project: learn the workflow, conventions, and find assigned work",
    status: "in_progress",
    dev_type: "docs",
    priority: "P1",
    creator: developer,
    ...
    createdAt: today,
    notes:
      "Generated by coding init for a new developer joining an existing Coding project",
  });
}
...
\`${developer}\` just ran \`coding init\` on a fresh clone, saw "Developer
initialized", and will now start asking you questions in chat. This joiner task
exists under \`.coding/tasks/\`; when they want to work on it, they should
start it from a session that provides Coding session identity.

Your job is to orient them to Coding. Don't dump all of this at them — open
...
- **Three phases**: Plan (brainstorm → \`prd.md\`) → Execute (code + check) →
  Finish (capture + wrap). Full reference: \`.coding/workflow.md\`.
...
- **\`/coding:continue\`** loads the Phase Index, reads \`prd.md\` + recent
  activity, and routes to the right skill (\`coding-brainstorm\` for planning,
  \`coding-implement\` for coding, \`coding-check\` for verification).
- **\`coding-implement\` sub-agent** is spawned when code needs to be written.
  ...
  conventions.
- **\`coding-check\` sub-agent** follows the same pattern with \`check.jsonl\`
...
- \`.coding/.runtime/sessions/<session>.json\` — session active-task state, gitignored
- \`.coding/tasks/<task>/{implement,check}.jsonl\` — per-task context manifests
- \`.coding/spec/\` — project-wide conventions (source of truth)
- \`.coding/workspace/${developer}/journal-*.md\` — their session log,
  rotated at ~2000 lines

### 3. This project's actual conventions

- Summarize \`.coding/spec/\` for them — what coding conventions this
  specific team enforces.
- Point at the last 5 entries in \`.coding/tasks/archive/\` as a rhythm
...
- Check if \`.coding/workspace/${developer}/\` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run \`${pythonCmd} ./.coding/scripts/task.py list --assignee ${developer}\` to
...
P3 task or a typo fix and run the full cycle together: \`/coding:continue\`
→ you implement via sub-agents → \`/coding:finish-work\`.
...
\`\`\`bash
${pythonCmd} ./.coding/scripts/task.py finish
${pythonCmd} ./.coding/scripts/task.py archive 00-join-${slug}
\`\`\`
...
"Welcome! Your \`coding init\` set me up to onboard you to this project. I
...
/**
 * Create joiner onboarding task for a new developer on an existing Coding
...
/**
 * Handle re-init when .coding/ already exists.
...
async function handleReinit(
    ...
    const { action } = await inquirer.prompt<{ action: string }>([
      {
        type: "list",
        name: "action",
        message: "Coding is already initialized. What would you like to do?",
        choices: [
        ...
        if (platformId) {
          if (configuredPlatforms.has(platformId)) {
            ...
            if (platformId === "claude-code" && options.withStatusline) {
              console.log(
                chalk.gray(
                  "   ↳ Coding statusLine installed (--with-statusline)",
                ),
              );
            }
          }
  ...
  if (doAddDeveloper) {
    ...
    // joiner onboarding onto an existing Coding project.
    ...
    try {
      ...
      console.log(
        chalk.gray(
          `  ${pythonCmd} .coding/scripts/init_developer.py ${devName}`,
        ),
      );
    }
  ...
  const answer = await inquirer.prompt<{ withStatusline: boolean }>([
    {
      ...
      name: "withStatusline",
      message:
        "Install Coding statusLine for Claude Code? (status bar: model, context, branch, rate limits)",
      default: false,
    },
  ]);
...
interface InitOptions {
  ...
  registry?: string;
  monorepo?: boolean;
  /** Claude Code only: install the opt-in Coding statusLine (--with-statusline) */
...
export async function init(options: InitOptions): Promise<void> {
  ...
  // (Claude/Codex/OpenCode session histories etc.) into the coding hash
  // manifest, and a subsequent `coding uninstall` would wipe it.
  ...
  // (.coding/ exists, .developer missing) apart from "creator first init".
  const hadDeveloperFileAtStart = fs.existsSync(
    path.join(cwd, DIR_NAMES.WORKFLOW, FILE_NAMES.DEVELOPER),
  );

  // Generate ASCII art banner dynamically using FIGlet "Rebel" font
  const banner = figlet.textSync("Coding", { font: "Rebel" });
  ...
  // ==========================================================================
  // Re-init fast path: skip full flow when .coding/ already exists
  // ==========================================================================

  // Aborted-init recovery (issue #204): if .coding/ exists but tasks/ is
  ...
  if (!developerName && !options.yes) {
    ...
    console.log(
      chalk.gray(
        "\nCoding supports team collaboration - each developer has their own\n" +
  ...
  if (options.monorepo !== false) {
    ...
    if (options.monorepo === true && !detected) {
      ...
      console.log("");
      console.log("To configure manually, add to .coding/config.yaml:");
      console.log("");
          ...
          for (const pkg of detected) {
            const { specSource } = await inquirer.prompt<{
            ...
            }>([
              {
                ...
                choices: [
                  { name: "From scratch (Coding default)", value: "blank" },
  ...
  if (selectedTemplate) {
    ...
    if (result.success) {
      ...
      const retryCmd = registry
        ? `coding init --registry ${registry.gigetSource} --template ${selectedTemplate}`
        : `coding init --template ${selectedTemplate}`;
    ...
    if (result.success) {
      ...
      console.log(
        chalk.gray(
          `   You can retry later: coding init --registry ${registry.gigetSource}`,
        ),
      );
    }
  }
  ...
  try {
    ...
    for (const tool of tools) {
      ...
      if (platformId) {
        ...
        if (platformId === "claude-code" && options.withStatusline) {
          console.log(
            chalk.gray("   ↳ Coding statusLine installed (--with-statusline)"),
          );
        }
      }
    }
  ...
  // `.coding/workflow.md` hash entry so `coding update` classifies it as
  ...
  if (developerName) {
    ...
    //
    // Tasks-empty fallback (issue #204): if .coding/ exists but tasks dir is
