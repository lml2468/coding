...
function binaryCheckCommand(
  ...
  return platform === "win32" ? "where coding" : "which coding";
}
...
function troubleshooting(plan: UpgradeCommandPlan): string {
  return [
    ...
    `- Check which Coding binary your shell resolves: ${plan.binaryCheckCommand}`,
    "- If this is a permissions error, fix your Node/npm install or npm prefix; Coding does not run sudo.",
    "- If npm reports an existing binary or locked file, resolve that npm error manually; Coding does not run --force.",
  ].join("\n");
}
...
export async function upgrade(
  ...
  console.log(chalk.cyan(`Upgrading Coding CLI to ${plan.target}`));
  ...
  console.log(chalk.green("\n✓ Coding CLI upgrade completed"));
  console.log(chalk.gray("Run: coding --version"));
