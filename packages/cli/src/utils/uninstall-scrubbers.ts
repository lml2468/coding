...
/**
 * Coding-specific values written by the Pi configurator.
 *
 * The `extensions`/`skills`/`prompts` arrays are paths relative to `.pi/`. We
 * remove the exact entries that the Pi configurator emits.
 */
const PI_CODING_EXTENSION = "./extensions/coding/index.ts";
const PI_CODING_SKILLS = "./skills";
const PI_CODING_PROMPTS = "./prompts";
const PI_SUBAGENTS_PACKAGE = "npm:pi-subagents";

function isCodingPiEntry(value: unknown, target: string): boolean {
  return typeof value === "string" && value === target;
}

/**
 * Scrub `.pi/settings.json`:
 * - drop `enableSkillCommands` (coding-flagged)
 * - remove coding entries from `extensions`/`skills`/`prompts` arrays
 * - remove coding-managed `packages["npm:pi-subagents"]` isolation override
...
export function scrubPiSettings(content: string): ScrubResult {
  ...
  const arrayCleanups: [string, string][] = [
    ["extensions", PI_CODING_EXTENSION],
    ["skills", PI_CODING_SKILLS],
    ["prompts", PI_CODING_PROMPTS],
  ];
  for (const [key, target] of arrayCleanups) {
    const arr = root[key];
    if (!Array.isArray(arr)) continue;
    const filtered = arr.filter((v) => !isCodingPiEntry(v, target));
 ...
 *
 * The current coding-emitted file has two distinct chunks:
 ...
 *  - any line that is *only* a comment introduced by coding (the entire file
 *    as shipped is comments + that one assignment)
 *  - blank lines that surrounded those removals
 *
 * If the user added their own non-coding lines, they are preserved as-is.
...
export function scrubCodexConfigToml(content: string): ScrubResult {
  const codingCommentMarkers = [
    "Project-scoped Codex defaults for Coding workflows.",
  ...
  // A comment line is "coding-known" if its content (after `#` and spaces)
  // matches one of the known marker strings exactly OR is an empty `#` line.
  function isCodingCommentLine(line: string): boolean {
    const trimmed = line.trim();
    if (!trimmed.startsWith("#")) return false;
    const inner = trimmed.replace(/^#+\s?/, "").trim();
    if (inner.length === 0) return true; // bare `#` line inside coding block
    return codingCommentMarkers.some((m) => inner === m);
  }

  function isCodingAssignment(line: string): boolean {
  ...
  for (const rawLine of content.split(/\r?\n/)) {
    if (isCodingAssignment(rawLine) || isCodingCommentLine(rawLine)) {
      continue; // drop
    }
