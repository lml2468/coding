/**
 * Remote template fetcher for Coding CLI
 *
...
async function cloneRegistryRef(
  ...
  const dir = await fs.promises.mkdtemp(
    path.join(os.tmpdir(), "coding-registry-"),
  );
  try {
    try {
      await runGit([
...
export async function downloadWithStrategy(
  ...
  if (strategy === "overwrite" && exists) {
    const tempDir = path.join(os.tmpdir(), `coding-template-${Date.now()}`);
    try {
      await withTimeout(
 ...
 * files as update-template entries under `.coding/spec/**`.
 */
export async function fetchRegistrySpecTemplates(
  ...
  const tempRoot = await fs.promises.mkdtemp(
    path.join(os.tmpdir(), "coding-registry-spec-"),
  );
  try {
    ...
    return {
      ...
      files: collectDirectoryFiles(
        path.join(tempRoot, ".coding", "spec"),
        ".coding/spec",
      ),
    };
  } finally {
