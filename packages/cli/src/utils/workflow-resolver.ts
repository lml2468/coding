 ...
 *
 * Centralizes how `coding init --workflow` and `coding workflow` discover and
...
import { workflowMdTemplate } from "../templates/coding/index.js";
import {
  TIMEOUTS,
  TEMPLATE_INDEX_URL,
 ...
 *
 * Treated as Coding-managed for hash-tracking: when this id is selected by
 * `init --workflow` or `coding workflow`, `.coding/workflow.md` stays in
 ...
 *   `bundled:coding/workflow.md` for the native virtual entry.
 */
...
function nativeListingEntry(): WorkflowTemplateListing {
  return {
    ...
    type: "workflow",
    name: "Native Coding Workflow",
    description:
      "Default Coding Plan / Execute / Finish workflow bundled with the CLI",
    path: "bundled:coding/workflow.md",
    source: "bundled",
  };
}
...
async function fetchWorkflowFileGit(
  ...
  const dir = await fs.promises.mkdtemp(
    path.join(os.tmpdir(), "coding-workflow-"),
  );
