...
/**
 * Patterns to exclude from hash tracking (only applied to the .coding/ walk).
 */
const EXCLUDE_FROM_HASH = [
  ...
  ".coding/spec/", // User-customized spec files
 ...
 * uninstall removes `.coding/` wholesale via `rm -rf` regardless of manifest
 * content — accuracy there doesn't affect data-loss, only `coding update`
...
export function initializeHashes(
  ...
  if (trackedPaths) {
    for (const relativePath of trackedPaths) {
      // `.coding/` paths are handled by the walk below — don't double-track.
      if (relativePath.startsWith(".coding/") || relativePath === ".coding") {
        continue;
      }
  ...
  // .coding/ workflow tree: still walked recursively. Accuracy here is for
  // `coding update`'s 3-way merge of workflow.md / config.yaml / scripts;
  // uninstall removes .coding/ wholesale so it does not matter for the
  // data-loss bug this contract addresses.
  const files = collectFiles(cwd, ".coding");
