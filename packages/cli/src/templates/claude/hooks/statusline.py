...
# -*- coding: utf-8 -*-
"""
Coding StatusLine — project-level status display for Claude Code.

Reads Claude Code session JSON from stdin + Coding task data from filesystem.
Outputs 1-2 lines:
...
def _find_coding_dir() -> Path | None:
    """Walk up from cwd to find .coding/ directory."""
    current = Path.cwd()
    for parent in [current, *current.parents]:
        candidate = parent / ".coding"
        if candidate.is_dir():
            return candidate
    return None


def _get_current_task(coding_dir: Path) -> dict | None:
    """Load current task info through Coding' active task resolver."""
    return _get_current_task_for_input(coding_dir, {})


def _get_current_task_for_input(coding_dir: Path, cc_data: dict) -> dict | None:
    ...
    active = resolve_active_task(coding_dir.parent, cc_data, platform="claude")
    if not active.task_path:
        return None

    task_path = _resolve_task_dir(coding_dir, active.task_path)
    if active.stale:
        return {
...
def _count_active_tasks(coding_dir: Path) -> int:
    """Count non-archived task directories with valid task.json."""
    tasks_dir = coding_dir / "tasks"
...
def _get_developer(coding_dir: Path) -> str:
    content = _read_text(coding_dir / ".developer")
    if not content:
        return "unknown"
...
def main() -> None:
    ...
    coding_dir = _find_coding_dir()
    SEP = " \033[90m·\033[0m "

    # --- Coding data ---
    task = _get_current_task_for_input(coding_dir, cc_data) if coding_dir else None
    dev = _get_developer(coding_dir) if coding_dir else ""
    task_count = _count_active_tasks(coding_dir) if coding_dir else 0
