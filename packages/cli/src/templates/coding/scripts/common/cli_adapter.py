...
class CLIAdapter:
    ...
    _AGENT_NAME_MAP: ClassVar[dict[Platform, dict[str, str]]] = {
        ...
        "opencode": {
            "plan": "coding-plan",  # 'plan' is built-in in OpenCode
        },
    }
    ...
    def get_agent_name(self, agent: str) -> str:
        ...
        Returns:
            Platform-specific agent name (e.g., 'coding-plan' for OpenCode)
        """
    ...
    def get_commands_path(self, project_root: Path, *parts: str) -> Path:
        ...
        Args:
            project_root: Project root directory
            *parts: Additional path parts (e.g., 'coding', 'finish-work.md')

        Returns:
            Path to commands directory or file

        Note:
            Cursor uses prefix naming: .cursor/commands/coding-<name>.md
            Antigravity uses workflow directory: .agent/workflows/<name>.md
            Devin uses workflow directory: .devin/workflows/coding-<name>.md
            Copilot uses prompt files: .github/prompts/<name>.prompt.md
            Pi uses prompt templates: .pi/prompts/coding-<name>.md
            Claude/OpenCode use subdirectory: .claude/commands/coding/<name>.md
        """
        if self.platform == "pi":
            prompts_dir = self.get_config_dir(project_root) / "prompts"
            if not parts:
                return prompts_dir
            if len(parts) >= 2 and parts[0] == "coding":
                filename = parts[-1]
                if filename.endswith(".md"):
                    filename = filename[:-3]
                return prompts_dir / f"coding-{filename}.md"
        ...
        if self.platform == "omp":
            ...
            if len(parts) >= 2 and parts[0] == "coding":
                ...
                if filename.endswith(".md"):
                    filename = filename[:-3]
                return commands_dir / f"coding-{filename}.md"
        ...
        if self.platform == "devin":
            ...
            if len(parts) >= 2 and parts[0] == "coding":
                filename = parts[-1]
                return workflow_dir / f"coding-{filename}"
        ...
        if self.platform == "cursor" and len(parts) >= 2 and parts[0] == "coding":
            # Convert coding/<name>.md to coding-<name>.md
            filename = parts[-1]
            return (
                self.get_config_dir(project_root) / "commands" / f"coding-{filename}"
            )

        return self.get_config_dir(project_root) / "commands" / Path(*parts)

    def get_coding_command_path(self, name: str) -> str:
        """Get relative path to a coding command file.
        ...
        Note:
            Cursor: .cursor/commands/coding-<name>.md
            Codex: .agents/skills/coding-<name>/SKILL.md
            Kiro: .kiro/skills/coding-<name>/SKILL.md
            Gemini: .gemini/commands/coding/<name>.toml
            Antigravity: .agent/workflows/<name>.md
            Devin: .devin/workflows/coding-<name>.md
            Pi: .pi/prompts/coding-<name>.md
            Others: .{platform}/commands/coding/<name>.md
        """
        if self.platform == "cursor":
            return f".cursor/commands/coding-{name}.md"
        elif self.platform == "codex":
            # 0.5.0-beta.0 renamed all skill dirs to add the `coding-` prefix
            # (see that release's manifest for the 60+ rename entries).
            return f".agents/skills/coding-{name}/SKILL.md"
        elif self.platform == "kiro":
            return f".kiro/skills/coding-{name}/SKILL.md"
        elif self.platform == "gemini":
            return f".gemini/commands/coding/{name}.toml"
        elif self.platform == "antigravity":
            return f".agent/workflows/{name}.md"
        elif self.platform == "devin":
            return f".devin/workflows/coding-{name}.md"
        elif self.platform == "kilo":
            ...
            return f".omp/commands/coding-{name}.md"
        else:
            return f"{self.config_dir_name}/commands/coding/{name}.md"
...
def detect_platform(project_root: Path) -> Platform:
    ...
    Detection order:
    1. CODING_PLATFORM environment variable (if set)
    ...
    env_platform = os.environ.get("CODING_PLATFORM", "").lower()
    # Deprecated alias: Windsurf was renamed to Devin.
    ...
    # as devin for back-compat until users migrate via `coding update --migrate`).
    if (
    ...
    # (.agents/skills/coding-* dirs) and no explicit platform config dir.
    ...
    if agents_skills.is_dir() and not _has_other_platform_dir(
        ...
        try:
            for entry in agents_skills.iterdir():
                if entry.is_dir() and entry.name.startswith("coding-"):
                    return "codex"
        except OSError:
            pass
