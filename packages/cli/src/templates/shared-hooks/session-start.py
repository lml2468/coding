...
First visible reply: say once in Chinese that Coding SessionStart context is loaded, then answer directly.
...
def _persist_context_key_for_bash(context_key: str | None) -> None:
    """Expose Coding session identity to later Claude Code Bash commands.
    ...
    try:
        with open(env_file, "a", encoding="utf-8") as handle:
            handle.write(f"export CODING_CONTEXT_ID={shlex.quote(context_key)}\n")
    except OSError:
        pass  # Optional shell bridge; keep session-start non-fatal.


def _resolve_active_task(coding_dir: Path, input_data: dict):
    scripts_dir = coding_dir / "scripts"
    if str(scripts_dir) not in sys.path:
        sys.path.insert(0, str(scripts_dir))
...
def run_script(script_path: Path, context_key: str | None = None) -> str:
    try:
        if script_path.suffix == ".py":
            ...
            if context_key:
                env["CODING_CONTEXT_ID"] = context_key
...
def _normalize_task_ref(task_ref: str) -> str:
    ...
    if normalized.startswith("tasks/"):
        return f".coding/{normalized}"

    return normalized


def _resolve_task_dir(coding_dir: Path, task_ref: str) -> Path:
    normalized = _normalize_task_ref(task_ref)
    path_obj = Path(normalized)
    if path_obj.is_absolute():
        return path_obj
    if normalized.startswith(".coding/"):
        return coding_dir.parent / path_obj
    return coding_dir / "tasks" / path_obj


def _get_task_status(coding_dir: Path, input_data: dict) -> str:
    """Return compact active-task status, artifact presence, and next action."""
    active = _resolve_active_task(coding_dir, input_data)

    if not active.task_path:
        return (
            "Status: NO ACTIVE TASK\n"
            "Next-Action: Classify the current turn before creating any Coding task. "
            "Simple conversation / small task asks only whether this turn should create a Coding task. "
            "Complex task asks whether task creation and planning are allowed."
        )

    task_ref = active.task_path
    task_dir = _resolve_task_dir(coding_dir, task_ref)
    if active.stale or not task_dir.is_dir():
        return (
            f"Status: STALE POINTER\nTask: {task_ref}\n"
            f"Next-Action: Run `python3 ./.coding/scripts/task.py finish` to clear the stale pointer, "
    ...
    if task_status == "planning" and not has_prd:
        return (
            f"Status: PLANNING\nTask: {task_title}\n"
            f"Present: {present_line}\n"
            "Next-Action: Load `coding-brainstorm` and write `prd.md`. Stay in planning."
        )
...
def _load_coding_config(coding_dir: Path, input_data: dict) -> tuple:
    """Load Coding config for session-start decisions.
...
def _check_legacy_spec(coding_dir: Path, is_mono: bool, packages: dict) -> str | None:
...
def _collect_spec_index_paths(coding_dir: Path, allowed_pkgs: set | None) -> list[str]:
    paths: list[str] = []
    guides_index = coding_dir / "spec" / "guides" / "index.md"
    if guides_index.is_file():
        paths.append(".coding/spec/guides/index.md")
    ...
    for sub in sorted(spec_dir.iterdir()):
        ...
        if index_file.is_file():
            paths.append(f".coding/spec/{sub.name}/index.md")
            continue
        ...
        for nested in sorted(sub.iterdir()):
            ...
            if nested_index.is_file():
                paths.append(f".coding/spec/{sub.name}/{nested.name}/index.md")
...
def _build_compact_current_state(
    ...
    active = _resolve_active_task(coding_dir, input_data)
    if active.task_path:
        task_dir = _resolve_task_dir(coding_dir, active.task_path)
        status = "unknown"
    ...
    if get_tasks_dir and iter_active_tasks:
        try:
            ...
            lines.append(
                f"Active tasks: {task_count} total. Use `python3 ./.coding/scripts/task.py list --mine` only if needed."
            )
        except Exception:
...
def _build_workflow_overview(workflow_path: Path) -> str:
    ...
    out_lines = [
        ...
        "Full guide: .coding/workflow.md. Step detail: `python3 ./.coding/scripts/get_context.py --mode phase --step <X.Y>`.",
        "",
    ]
...
def main():
    ...
    coding_dir = project_dir / ".coding"
    context_key = _resolve_context_key(coding_dir, hook_input)
    _persist_context_key_for_bash(context_key)

    # Load config for scope filtering and legacy detection
    is_mono, packages, scope_config, task_pkg, default_pkg = _load_coding_config(
        coding_dir,
        hook_input,
    )
    ...
    spec_index_paths = _collect_spec_index_paths(coding_dir, allowed_pkgs)

    output.write("""<session-context>
Coding compact SessionStart context. Use it to orient the session; load details on demand.
</session-context>
...
""")
    ...
    legacy_warning = _check_legacy_spec(coding_dir, is_mono, packages)
    if legacy_warning:
        output.write(f"<migration-warning>\n{legacy_warning}\n</migration-warning>\n\n")

    output.write("<current-state>\n")
    output.write(_build_compact_current_state(coding_dir, hook_input, spec_index_paths))
    output.write("\n</current-state>\n\n")

    output.write("<coding-workflow>\n")
    output.write(_build_workflow_overview(coding_dir / "workflow.md"))
    output.write("\n</coding-workflow>\n\n")
    ...
    task_status = _get_task_status(coding_dir, hook_input)
