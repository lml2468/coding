...
**Problem**: Python's if/elif/else chains have no compile-time exhaustive check. When you add a new value to a `Literal` type (e.g., a task phase), existing if/elif/else chains silently fall through to `else` with wrong defaults.

**Symptom**: The new value works partially — some methods return the default branch instead of the value-specific behavior. No error is raised.

**Example**:
```python
# BAD: "check" falls through to else, returns the "implement" label
@property
def phase_label(self) -> str:
    if self.phase == "brainstorm":
        return "Brainstorm"
    else:
        return "Implement"  # "check" silently gets "Implement"!
...
**Real example**: `coding update` had a manual `files.set()` list for 11 scripts that `getAllScripts()` already tracked. Fix: replaced the manual list with a `for..of getAllScripts()` loop. See `update.ts` refactor in v0.4.0-beta.3.

---

## Template File Registration (Coding-specific)

When adding new files to `src/templates/coding/scripts/`:

**Single registration point**: `src/templates/coding/index.ts`
...
**Why this matters**: Without registration in `getAllScripts()`, `coding update` won't sync the file to user projects. Bug fixes and features won't propagate.

**History**: Before v0.4.0-beta.3, `update.ts` had its own hand-maintained file list that frequently fell out of sync with `getAllScripts()`. This caused 11 Python files to be silently skipped during `coding update`. The fix was to eliminate the duplicate list and use `getAllScripts()` as the single source of truth.

### Quick Checklist for New Scripts

```bash
# After adding a new .py file, verify it's in getAllScripts():
grep -l "newFileName" src/templates/coding/index.ts  # Should match
```

### Template Sync Convention

`.coding/scripts/` (dogfooded) and `packages/cli/src/templates/coding/scripts/` (template) must stay identical. After editing `.coding/scripts/`, always sync:

```bash
rsync -av --delete --exclude='__pycache__' .coding/scripts/ packages/cli/src/templates/coding/scripts/
```

**Gotcha**: Running rsync with wrong source/destination paths can create nested garbage directories (e.g., `.coding/scripts/packages/cli/...`). Always double-check paths before running.
