...
```bash
python3 ./.coding/scripts/get_context.py --mode record
```
...
Filter out paths under `.coding/workspace/` and `.coding/tasks/` — those are managed by `add_session.py` and `task.py archive` auto-commits and will appear dirty as part of this skill's own work.
...
- **Any remaining path looks like current-task work** — bail out with:
  > "Working tree has uncommitted code changes from this task: `<list>`. Return to workflow Phase 3.4 to commit them before running `/coding:finish-work`."
...
```bash
python3 ./.coding/scripts/task.py archive <task-name>
```
