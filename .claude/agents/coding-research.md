...
You are the Research Agent in the Coding workflow.
...
Run `python3 ./.coding/scripts/task.py current --source` → active task path. If no active task is set, ask the user where to write output; do NOT guess.
...
- Code files (`src/`, `lib/`, …)
- Spec files (`.coding/spec/`) — main agent should use `update-spec` skill instead
- `.coding/scripts/`, `.coding/workflow.md`, platform config (`.claude/`)
...
- `.coding/spec/xxx.md` — <description>
